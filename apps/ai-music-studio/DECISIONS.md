...
## ADR-027 — Shared nginx Gateway Instead of Planned Caddy
Status: Accepted (Phase 14)

The actual public gateway is a shared Hetzner nginx 1.24/Certbot host, not Caddy. The Studio production stack stays isolated in `hg-studio-prod` on loopback ports, and a dedicated, key-restricted remote SSH listener at gateway loopback `127.0.0.1:13211` is the narrowest route to its web port. Only a new `studio.hungreegoat.com` nginx vhost and certificate are permitted; existing broadcast and sibling vhosts/tunnel identities are untouched. Studio's web server proxies its own same-origin API, so neither the API nor Windmill is publicly bound. The nginx templates are syntax-tested locally. Installation awaits privileged gateway access; resetting the shared root password or rebooting into rescue would breach the protected boundary.

## ADR-028 — Isolated Production Monitoring and Encrypted Recovery
Status: Accepted (Phase 14)

A separate `hg-studio-prod` project uses 0600 private credentials, resource-limited containers, loopback-only web/API, and an opt-in pinned Uptime Kuma 2.5.5 service with no Docker socket. Its internal web/API/Windmill checks are live; a public HTTPS check remains visibly down until the gateway route exists. A read-only Studio monitor checks worker ping, provider state, storage, local/off-site backup age, and container/HTTP health every two minutes. Daily backups encrypt two PostgreSQL custom dumps, immutable assets, Studio secrets and configuration, and Kuma's consistent SQLite dump; monthly restore checks use disposable databases. Complete encrypted sets are copied and checksummed to a private directory in the unprivileged gateway account, without modifying gateway services. The encryption passphrase is deliberately excluded from both copies and requires separate escrow. No backup or monitor touches broadcast volumes or services.

## ADR-029 — Vercel Deployment Path Dropped
Status: Accepted (Phase 14)

The owner decided against pursuing the separately connected `hungreegoat-ai-music-studio` Vercel project (H14-03): the Compose/nginx production path in ADR-027 is canonical and already fully implements the product, a Vercel frontend cannot reach the private Studio API without the same gateway/DNS authority anyway, and no Vercel token/session was ever available to debug the repeated build failures. No in-repo Vercel configuration exists (`vercel.json`/`.vercel` were never committed), so no code change was needed. This is a closed decision, not an open blocker; do not re-open H14-03 or re-attempt Vercel debugging without a new owner request.
