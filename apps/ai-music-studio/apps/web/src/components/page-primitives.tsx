...
export function PageHeading({ eyebrow = "YOUR STUDIO", title, description, children }: { eyebrow?: string; title: string; description: string; children?: React.ReactNode }) {
  return <div className="mb-8 flex flex-wrap items-end justify-between gap-5"><div className="min-w-0 max-w-2xl space-y-2"><p className="text-[10px] font-semibold tracking-[.18em] text-muted-foreground">{eyebrow}</p><h1 className="break-words text-3xl font-semibold tracking-tight md:text-4xl">{title}</h1><p className="break-words text-sm leading-relaxed text-muted-foreground">{description}</p></div>{children}</div>;
}
