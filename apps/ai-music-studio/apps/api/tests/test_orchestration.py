...
def test_stem_separation_and_mix_are_leased_immutable_and_scoped(orchestration, tmp_path):
    ...
    source = browser.post(
        f"/api/v1/projects/{project_id}/assets/upload",
        files={"file": ("source.wav", original, "audio/wav")},
        data={"song_id": song["id"]},
    ).json()
    ...
    assert queued.status_code == 202, queued.text
    assert (
        browser.post(url, json={"idempotency_key": key}).json()["job_id"] == queued.json()["job_id"]
    )
    job_id = UUID(queued.json()["job_id"])
    ...
    completed = browser.post(
        base + "/complete",
        headers=worker_headers(lease),
        json={"outputs": [], "result": {"stem_set_id": stem_set_id, "asset_ids": stems}},
    )
    ...
    assert all(
        browser.get(f"/api/v1/assets/{asset_id}/lineage").json()["items"][0]["parent_asset_id"]
        == source["id"]
        for asset_id in stems.values()
    )
    mix_url = f"/api/v1/stem-sets/{stem_set_id}/mix-versions"
    levels = {"vocals": 0.8, "drums": 1.0, "bass": 0.4, "other": 0.0}
    assert (
        browser.post(
            mix_url, json={"idempotency_key": str(uuid4()), "levels": {"vocals": 1.0}}
        ).status_code
        == 422
    )
    ...
    mixed = browser.put(
        mix_base + "/mix-audio",
        content=flac,
        headers={**worker_headers(mix_lease), "Content-Type": "audio/flac"},
    )
    assert mixed.status_code == 200, mixed.text
    mix_asset = mixed.json()["asset_id"]
    done = browser.post(
        mix_base + "/complete",
        headers=worker_headers(mix_lease),
        json={"outputs": [], "result": {"stem_set_id": stem_set_id, "asset_id": mix_asset}},
    )
    ...
    assert (
        len(versions) == 1
        and versions[0]["asset_id"] == mix_asset
        and versions[0]["levels"] == levels
    )
    parents = {
        row["parent_asset_id"]
        for row in browser.get(f"/api/v1/assets/{mix_asset}/lineage").json()["items"]
    }
