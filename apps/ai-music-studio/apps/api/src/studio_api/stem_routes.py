...
from studio_api.stems import LABELS, CreateMix, CreateStemSet, MixInput
...
def stem_set_public(db: DB, row: StemSet) -> dict:
    stems = db.scalars(
        select(Stem)
        .where(Stem.stem_set_id == row.id, Stem.workspace_id == row.workspace_id)
        .order_by(Stem.label)
    ).all()
    return {
        **public(row, ("id", "source_asset_id", "engine", "engine_version", "created_at")),
        "stems": [public(stem, ("id", "label", "asset_id", "created_at")) for stem in stems],
    }
...
def list_stem_sets(
    ...
    rows = db.scalars(
        select(StemSet)
        .where(StemSet.source_asset_id == asset_id, StemSet.workspace_id == auth.workspace_id)
        .order_by(StemSet.created_at.desc())
        .limit(limit)
        .offset(offset)
    )
...
def request_stem_set(asset_id: UUID, data: CreateStemSet, auth: Auth, db: DB) -> dict:
    ...
    if asset.duration_seconds > 180:
        raise fail(
            422, "source_too_long", "CPU stem separation is limited to three-minute audio assets"
        )
    try:
...
def list_mix_versions(
    stem_set_id: UUID, auth: Auth, db: DB, limit: Limit = 50, offset: Offset = 0
) -> dict:
    scoped(db, StemSet, stem_set_id, auth.workspace_id)
    rows = db.scalars(
        select(MixVersion)
        .where(MixVersion.stem_set_id == stem_set_id, MixVersion.workspace_id == auth.workspace_id)
        .order_by(MixVersion.version.desc())
        .limit(limit)
        .offset(offset)
    )
    return {
        "items": [
            public(row, ("id", "stem_set_id", "asset_id", "version", "levels", "created_at"))
            for row in rows
        ]
    }
...
def request_mix(stem_set_id: UUID, data: CreateMix, auth: Auth, db: DB) -> dict:
    ...
    stems = db.scalars(
        select(Stem).where(Stem.stem_set_id == stem_set.id, Stem.workspace_id == auth.workspace_id)
    ).all()
    if {row.label for row in stems} != set(LABELS):
        raise fail(409, "stems_incomplete", "All four stems must be ready before mixing")
    if set(data.levels) != set(LABELS) or any(
        not math.isfinite(value) or not 0 <= value <= 2 for value in data.levels.values()
    ):
