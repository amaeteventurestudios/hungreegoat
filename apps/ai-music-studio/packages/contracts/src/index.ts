...
export interface ArrangementSection { kind:"intro"|"verse"|"chorus"|"bridge"|"outro"; name:string; duration_seconds:number; energy:number; instrumentation:string[]; production_notes:string; vocal_instructions:string }
export interface ArrangementRevision { id:string; version_id:string; revision:number; asset_id:string; sections:ArrangementSection[]; created_at:string }
export interface AudioAnalysis { id:string; asset_id:string; analyzer_version:string; detected_bpm:number|null; musical_key:string|null; measurements:{duration_seconds:number;sample_rate:number;channels:number;detected_bpm:number|null;musical_key:string|null;loudness_lufs:number|null;peaks:number[]}; created_at:string }
export interface TempoVersion { id:string; source_asset_id:string; asset_id:string; ratio:number; pitch_semitones:number; preserve_pitch:boolean; created_at:string }
