...
Windmill runtime identity is solved technically and is not a blocker. The two items below block public HTTPS acceptance and total-host-loss key recovery; isolated production verification and every other safe Studio task continue independently.
...
7. **Architecture-preserving alternatives considered:** Deploy on the existing gateway via dedicated SSH tunnel and nginx; use a direct public Studio host port; use Vercel for the Next.js shell; change DNS to another frontend; reuse the broadcast tunnel; use Hetzner reset/rescue.
8. **Why alternatives cannot satisfy safely now:** Direct host exposure lacks DNS/TLS and bypasses the intended gateway; Vercel has no Studio project/session and external DNS still points to nginx; DNS changes require an unavailable authenticated DNS control plane; broadcast tunnel reuse weakens isolation; root reset/rescue would affect unrelated tenants. The dedicated nginx route is ready but requires the unavailable privileged gateway operation.
