...
4. **Safe paths exhausted:** Fetched origin and verified the latest Studio checkpoint `6acffb2` on GitHub; inspected public GitHub commit statuses and deployment statuses, this checkout's Vercel files/session names, unauthenticated Vercel dashboard/deployment/REST endpoints, and ran the exact `npx vercel inspect ... --logs` diagnostic for an earlier failed Studio deployment. Local `npm run build`, typecheck, lint, production Docker build, and loopback-routed production-origin browser smoke all pass; the Studio Vercel status failed across multiple commits while the two sibling Vercel projects succeeded.
5. **Commands/endpoints/errors:** GitHub's commit-status API reports `Vercel – hungreegoat-ai-music-studio: failure` at `6acffb2` and identifies `dpl_CHbJ186BUG1vBG6UkSrbwtPUtJED`; sibling Vercel statuses are `success`. `CI=1 npx --yes vercel inspect dpl_GUr83rdQAxZKNx9z8XgktWfVAghZ --logs` returned `No existing credentials found`. The unauthenticated Vercel deployment metadata/events API returned 403 and the deployment URL redirected to account SSO.
...
## H14-04 — Live AI provider credential and paid-call proof

1. **Blocked requirement:** Verify a real paid AI Producer request and ElevenLabs music generation through the production provider adapters, rather than only fixture/contract tests.
2. **Exact external action:** An authorized owner of the relevant provider accounts must securely configure an active OpenAI/OpenRouter/Anthropic key for Producer and an ElevenLabs Music-capable key in Studio Settings, and authorize the minimal paid requests. No key should be sent in chat or committed.
3. **Why Codex cannot perform it:** Production `provider_configs` contains zero entries, no matching provider key names are present in the current process environment, and there is no authenticated external provider-account control plane here. ElevenLabs' Music API is available to paid users and its key is issued through the account dashboard; Codex cannot create a paid account or authorize payment.
4. **Safe paths exhausted:** Generic provider contracts, structured-output adapters, job/worker orchestration, fixture responses, secret encryption/redaction, model/health APIs, production smoke, and browser missing-credential states were verified. No paid request was sent with a fabricated key; the production monitor accurately reports `none_configured`.
5. **Commands/endpoints/errors:** Production PostgreSQL `SELECT count(*), count(*) FILTER (WHERE enabled) FROM provider_configs` returns `0|0`; `scripts/monitor-studio.py` reports `enabled_provider_health: none_configured`. The shell environment-name scan found no `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `OPENROUTER_API_KEY`, or `ELEVENLABS` key. The secure Settings UI is implemented but cannot invent a provider credential.
6. **Official references checked:** [ElevenLabs Music quickstart and paid-account/key requirement](https://elevenlabs.io/docs/eleven-api/guides/cookbooks/music), [ElevenLabs API key handling](https://elevenlabs.io/docs/overview/administration/workspaces/api-keys), and the [Music compose endpoint](https://elevenlabs.io/docs/api-reference/music/compose-detailed).
7. **Architecture-preserving alternatives considered:** Continue deterministic mocks/fixtures, use a local OSS music model behind the adapter, call the paid provider anonymously, or defer live paid proof until a legitimate key exists.
8. **Why alternatives cannot satisfy this exact proof:** Fixtures prove contract behavior but not the external paid service; an OSS engine could be a separate adapter but cannot prove the specified ElevenLabs integration; anonymous paid calls cannot succeed. Creating a paid provider account or authorizing its charges is outside Studio engineering authority.
9. **Independent work:** Production stack, local audio engines, security, browser/API coverage, backup/restore, and public gateway readiness proceed without provider keys. Once legitimate credentials exist, run the smallest real plan/generation smoke and verify immutable versions, playback, provider health, and source lineage.
