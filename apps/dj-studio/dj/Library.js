...
export class Library {
    ...
    async loadManifest() {
        try {
            // HUNGREE Goat patch: the upstream demo fetched the static data/manifest.json
            // (an Ardour-export index). We fetch the real HUNGREE Goat Library instead — see
            // hgc/dj.py::manifest() — already shaped with the `streamUrl` field this file's
            // own onLoadDirect path supports natively (upstream code below, unmodified).
            const manifestUrl = window.HGC_DJ_MANIFEST_URL || '/api/dj/manifest?station=lofi&playlist=workout';
            const resp = await fetch(manifestUrl, { credentials: 'same-origin' });
            const data = await resp.json();
            this.tracks = (data.tracks || []).map(t => ({ ...t, streamUrl: t.streamUrl }));
            this._filterAndRender();
            window.dispatchEvent(new CustomEvent('hgc:library-loaded', { detail: { count: this.tracks.length } }));
        } catch (err) {
