/* Explore: browse / search the real HUNGREE Goat catalog. Live: up next + today's schedule. */
...
export default function Library() {
  ...
  if (s.mode === 'explore') {
    ...
    return (<div className="sheet" role="dialog" aria-label="Library"><div className="sheet-h"><h4>Explore · {s.stations.find(x => x.id === s.station)?.short || 'Library'}</h4><span>{s.tracks.length} tracks</span><button className="x" onClick={close} aria-label="Close">✕</button></div>
      ...
      <ul className="tracks">{list.map(({ t, i }) => <li key={t.id} className={i === s.index ? 'on' : ''}><button onClick={() => { d({ type: 'explore-go', index: i }); d({ type: 'set', patch: { started: true } }); }}><img src={t.artwork} alt="" loading="lazy" /><span><b>{t.title}</b><small>{t.album || t.artist}</small></span><em>{fmt(t.duration)}</em></button></li>)}{!list.length && <li className="empty">No matches</li>}</ul></div>);
  }
  return (<div className="sheet" role="dialog" aria-label="Up next"><div className="sheet-h"><h4>Up next · {s.now?.station || 'Live'}</h4><button className="x" onClick={close} aria-label="Close">✕</button></div>
    ...
    <ul className="sched">{(s.schedule?.today || []).map((b, i) => <li key={i} className={b.on_now ? 'on' : ''}><em>{b.start}–{b.end}</em><span><b>{b.name}</b><small>{b.description}</small></span>{b.on_now && <i>now</i>}{b.next && !b.on_now && <i>next</i>}</li>)}</ul></div>);
}
