...
export default function Scene() {
  ...
  const [layers, setLayers] = useState<{ id: string; video?: string | null; image?: string | null }[]>(skin ? [{ id: layerId, video: asset.video, image: asset.image }] : []);
  const timer = useRef<number>();
  useEffect(() => { if (!skin) return; setLayers(l => l.some(x => x.id === layerId) ? l : [...l, { id: layerId, video: asset.video, image: asset.image }]); clearTimeout(timer.current); timer.current = window.setTimeout(() => setLayers(l => l.filter(x => x.id === layerId)), 1400); }, [layerId]); // eslint-disable-line
  return (
