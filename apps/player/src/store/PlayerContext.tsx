...
function reducer(s: State, a: Action): State {
  switch (a.type) {
    ...
    case 'skin': { const sk = s.skins.find(x => x.id === a.id); return { ...s, skinId: a.id, time: sk ? sk.time : s.time, timeAuto: false, ambience: sk && sk.ambience && Object.keys(sk.ambience).length ? { ...sk.ambience } : s.ambience }; }
    case 'time': { if (a.time === 'auto') { const t = autoTime(); const sk = pickSkin(s.skins, t, s.defaultSkin); return { ...s, timeAuto: true, time: t, skinId: sk ? sk.id : s.skinId }; } const sk = pickSkin(s.skins, a.time, s.defaultSkin); return { ...s, time: a.time, timeAuto: false, skinId: sk ? sk.id : s.skinId, ambience: sk && sk.ambience && Object.keys(sk.ambience).length ? { ...sk.ambience } : s.ambience }; }
...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { api.tracks(s.station).then(tracks => d({ type: 'set', patch: { tracks, index: 0 } })).catch(() => {}); }, [s.station]);
  useEffect(() => { api.skins().then(({ skins, def }) => { const merged = mergeSkins(skins); const st = stateRef.current; const keep = st.skinId && merged.find(x => x.id === st.skinId); const sk = keep ? merged.find(x => x.id === st.skinId)! : pickSkin(merged, st.time, def); d({ type: 'set', patch: { skins: merged, defaultSkin: def, skinId: sk ? sk.id : null, ambience: sk && sk.ambience && !keep ? { ...sk.ambience } : st.ambience } }); }).catch(() => { const sk = pickSkin(stateRef.current.skins, stateRef.current.time, null); if (sk && !stateRef.current.skinId) d({ type: 'set', patch: { skinId: sk.id, ambience: { ...(sk.ambience || {}) } } }); }); }, []);
