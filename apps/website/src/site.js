...
(() => {
  ...
  function applyYouTube(url) { ytUrl = url || ''; $$('.yt-link').forEach(a => { if (ytUrl) { a.href = ytUrl; a.hidden = false; } else { a.hidden = true; } }); }
  applyYouTube(ytUrl);
  ...
  const secs = ['home','now','stations','schedule','about'].map(id => document.getElementById(id)).filter(Boolean);
  const io = new IntersectionObserver(es => { es.forEach(en => { if (en.isIntersecting) $$('.nav a').forEach(a => a.classList.toggle('active', a.getAttribute('href') === '#' + (en.target.id === 'now' ? 'home' : en.target.id))); }); }, { rootMargin: '-40% 0px -50% 0px' });
  ...
  function render(s) {
    ...
    un.innerHTML = s.upNext.length ? s.upNext.slice(0, 5).map((t, i) => `<li><span class="n">${i + 1}</span><img src="${h(t.artwork)}" alt="" loading="lazy" width="44" height="44"><div><b>${h(t.title)}</b><span class="a">${h(t.artist || '')}</span></div><span class="d">${HG.fmt(t.duration)}</span></li>`).join('') : '<li class="empty">Up next is decided live by the station — check back in a moment.</li>';
    ...
    strip.innerHTML = (sch.today || []).length ? sch.today.map(b => `<div class="slot ${b.on_now ? 'now' : ''}">${b.on_now ? '<span class="badge">NOW PLAYING</span>' : ''}<span class="t">${h(b.start)} – ${h(b.end)}</span><b>${h(b.name)}</b><span>${h(b.description || '')}</span></div>`).join('') : '<div class="slot empty">No programme published for today.</div>';
