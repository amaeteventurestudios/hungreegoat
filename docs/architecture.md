...
## Broadcast core (pi-node-01, Raspberry Pi 4B)
...
- **FFmpeg** (`apps/control/hgc/streamer.py`) — hardware decode of the 720p loop + hardware H.264 encode, with a
...
- **HUNGREE Goat Control** (FastAPI, port 8090, LAN-only) — catalog (SQLite), scheduler, queue, alerts, uploads,
  YouTube setup, operator SPA (`static/`), and the public read-only router (`/v1/*`).
